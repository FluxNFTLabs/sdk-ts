...
const main = async () => {
  ...
  // init user2
  const wallet = ethwallet.Wallet.fromPrivateKey(Uint8Array.from(Buffer.from("741de4f8988ea941d3ff0287911ca4074e62b7d45c991a51186455366f10b544", 'hex')))
  ...
  // init msg
  const msg: fnfttypes.MsgDepositShares = {
    class_id: 'series',
    id: '0',
    sender: senderAddr,
    shares: '5',
  ...
  const msgAny: anytypes.Any = {
    type_url: `/${fnfttypes.MsgDepositShares.$type}`,
    value: fnfttypes.MsgDepositShares.encode(msg).finish(),
  }
  const msgJSON = {
    type: codectypemap[`/${fnfttypes.MsgDepositShares.$type}`],
    value: fnfttypes.MsgDepositShares.toJSON(msg)
  }

  // prep tx data
  const txBody: txtypes.TxBody = {
    messages: [msgAny, msgAny],
    memo: 'abc',
  ...
  let eip712SignDoc = getEIP712SignBytes(signDoc, [msgJSON, msgJSON],'')

  console.log(JSON.stringify(eip712SignDoc))
